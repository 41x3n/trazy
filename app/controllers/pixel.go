...
const (
	...
	gifImage       = "\x47\x49\x46\x38\x39\x61\x01\x00\x01\x00\x80\x00\x00\xff\xff\xff\x00\x00\x00\x2c\x00\x00\x00\x00\x01\x00\x01\x00\x00\x02\x02\x44\x01\x00\x3b"
)

func Pixel(c *gin.Context) {
	id := c.Query("id")
	c.Header("Content-Type", gifContentType)

	if id == "" {
		infoLogger.Println("Pixel request received from unknown user", c.ClientIP())
		c.Data(http.StatusOK, gifContentType, []byte(gifImage))
		return
	}
